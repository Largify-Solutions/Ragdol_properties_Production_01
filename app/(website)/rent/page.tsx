...
// Module-level cache for rent properties data
const CACHE_DURATION = 3 * 60 * 1000 // 3 minutes
let rentPropertiesCache: { data: any[] | null; timestamp: number } = { data: null, timestamp: 0 }

function getCachedRentProperties() {
  if (rentPropertiesCache.data && Date.now() - rentPropertiesCache.timestamp < CACHE_DURATION) {
    return rentPropertiesCache.data
  }
  return null
}

function setCachedRentProperties(data: any[]) {
  rentPropertiesCache.data = data
  rentPropertiesCache.timestamp = Date.now()
}

function clearRentPropertiesCache() {
  rentPropertiesCache.data = null
  rentPropertiesCache.timestamp = 0
}
...
async function fetchAllRentProperties() {
  ...
  const cached = getCachedRentProperties()
  ...
  try {
    ...
    setCachedRentProperties(allProperties)
...
useEffect(() => {
  ...
  useRealtimeMulti([
    { table: 'properties', onChange: () => { clearRentPropertiesCache(); loadProperties() } },
    { table: 'agent_properties', onChange: () => { clearRentPropertiesCache(); loadProperties() } }
  ]);
