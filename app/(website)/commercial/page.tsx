...
// Module-level cache for commercial properties data
const CACHE_DURATION = 3 * 60 * 1000 // 3 minutes
let commercialPropertiesCache: { data: any[] | null; timestamp: number } = { data: null, timestamp: 0 }

function getCachedCommercialProperties() {
  if (commercialPropertiesCache.data && Date.now() - commercialPropertiesCache.timestamp < CACHE_DURATION) {
    return commercialPropertiesCache.data
  }
  return null
}

function setCachedCommercialProperties(data: any[]) {
  commercialPropertiesCache.data = data
  commercialPropertiesCache.timestamp = Date.now()
}

function clearCommercialPropertiesCache() {
  commercialPropertiesCache.data = null
  commercialPropertiesCache.timestamp = 0
}
...
async function fetchAllProperties() {
  // Check cache first
  const cached = getCachedCommercialProperties()
  ...
  try {
    ...
    setCachedCommercialProperties(allProperties)
...
function CommercialPropertiesPageContent() {
  ...
  // Load ALL properties function (memoized with useCallback)
  ...
  useRealtimeMulti([
    { table: 'properties', onChange: () => { clearCommercialPropertiesCache(); loadProperties() } },
    { table: 'agent_properties', onChange: () => { clearCommercialPropertiesCache(); loadProperties() } }
  ]);
